...
if(isset($_SESSION["cart_time"])){
    $temps = $_SESSION["cart_time"];
    echo $temps;
}
...
function test(){
    let now = Date.now();
    let maintenant = Math.floor(now / 1000);
    let temps = <?php echo $temps ?>;
    let ecart = maintenant - temps;
    console.log(ecart);
...
setInterval(test, 1000); // Appelle la fonction test toutes les secondes (1000 milliseconds)
</script>
   <?php 
